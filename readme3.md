...
- `Game::load(filename)`: Parses a state file to restore the game's turns, hits, and platoon positions/states.
- `roll_dice()`: Generates a random integer (1-6).
- `ask_choice()`: Manages validated user input.

### Game State Loading
The program can be started from a specific state by passing the state file as an argument:
`./takethathill state.txt`

The state file follows this format:
- `turns:X`
- `hits:Y`
- `An:q,r,S` (where `n` is the platoon number, `q,r` are coordinates, and `S` is `F` for Fresh or `S` for Spent)
...
```
src
 |- main.cpp // Game loop, win/loss screens, and state loading entry point
 |- game.cpp // Game logic, combat resolution, and state parsing
